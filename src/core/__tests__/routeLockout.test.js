...
// everything; a Branch Accountant is blocked from HR / Settings / HO Control / Group
// dashboard but keeps all accounting/finance/reports/masters/tax routes.
...
describe('canReachRoute — hard route-level lockout', () => {
  test('full-menu roles reach everything, including the locked areas', () => {
    for (const r of ['/hr/employees', '/settings/users', '/ho/control', '/group-dashboard', '/finance/receipts']) {
  ...
  test('a Branch Accountant is blocked from out-of-scope admin areas by direct URL', () => {
    for (const r of ['/hr/employees', '/hr/payroll', '/settings/users', '/settings/company', '/ho/control', '/group-dashboard']) {
