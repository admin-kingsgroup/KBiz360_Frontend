...
export function BalanceSheetTallyLive({ branch }) {
  ...
  const { data: bs, isLoading, error } = useQuery({
    queryKey: ['bs-tally', brCodeOf(branch), range.from, range.to],
    queryFn: () => apiGet('/api/accounting/bs-tally', { branch: brCodeOf(branch) === 'ALL' ? '' : brCodeOf(branch), from: range.from, to: range.to }),
  });
