...
const empty = { title: '', scope: { level: 'module', ref: '', branches: ['ALL'] }, condition: { kind: 'field', coll: '', branchField: 'branch', field: '', op: 'set', value: '' }, severity: 'warn', message: '', fixLink: '', remark: '' };
...
export function RulesManager({ canManage = true }) {
  ...
  // Common (ALL) is the default; picking any branch drops ALL, clearing every branch restores it.
  const toggleBranch = (code) => setForm((f) => {
    const cur = f.scope.branches || ['ALL'];
    let next;
    if (code === 'ALL') next = ['ALL'];
    else {
      const picked = cur.filter((c) => c !== 'ALL');
      next = picked.includes(code) ? picked.filter((c) => c !== code) : [...picked, code];
      if (!next.length) next = ['ALL'];
    }
    return { ...f, scope: { ...f.scope, branches: next } };
  });
      ...
      {showAdd && (
        <PageSection title="New rule — saves as Inactive (Draft) until you Test & Activate">
          <div className="grid gap-3">
            ...
            <FormField label="Branches — Common applies everywhere; pick branches for a branch-specific rule">
              <div className="flex flex-wrap gap-2">
                {['ALL', ...BRANCH_CODES].map((code) => {
                  const on = (form.scope.branches || ['ALL']).includes(code);
                  return (
                    <button key={code} type="button" onClick={() => toggleBranch(code)}
                      ...
                      {code === 'ALL' ? 'Common (all branches)' : code}
                    </button>
                  );
                })}
              </div>
            ...
            : (() => {
                ...
                return (
                  <div key={r._id} className="rounded-lg border border-surface-border bg-surface p-3">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {!isCommonRule(r) && <Badge tone="info" size="sm">{r.scope.branches.join(' · ')}</Badge>}
                    </div>
              ...
              const sectionHead = (label, count) => (
                <div className="mt-2 flex items-center gap-2 first:mt-0">
                  ...
                  <span className="h-px grow bg-surface-border" />
                </div>
              );
              return (
                <div className="grid gap-2">
                  {grouped.common.length > 0 && (
                    <>
                      {sectionHead('Common Rules — all branches', grouped.common.length)}
                      {grouped.common.map(renderRule)}
                    </>
                  )}
                  {grouped.branches.map(({ branch, rules }) => (
                    <React.Fragment key={branch}>
                      {sectionHead(`${branch} — branch rules`, rules.length)}
