...
// The SIGN-OFF workbench (Rule 01: one certificate per ledger). Branch-wise
// (Rule 06: one branch at a time, never mixed). The TIER is fixed by the route —
// Certification ▸ Weekly / Monthly / Quarterly / Yearly Certification are
// separate menu entries rendering this page tier-locked. Per-ledger register
// grouped Parent Group → Sub-group → ledger. Opening a row works the certificate
// in a drawer: freeze → attach → exceptions → sign chain → (physical) scan-back.
// The read-only overview of the SAME scope lives on the Reconciliation Hub.
...
// never mixes tiers.
export function CertificationRegister({ branch: appBranch, setRoute, currentUser, tier: fixedTier }) {
  ...
  const tierAllowed = tiers.some((t) => t.key === tierKey) || granted.includes(certPathFor(tierKey));
