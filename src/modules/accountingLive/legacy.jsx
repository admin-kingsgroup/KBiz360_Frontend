      ...
      <div style={{ padding: 14 }}>
        ...
        <div style={{ ...card, padding: 10, boxShadow: 'none', border: '1px solid #dfe2e7' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11.5 }}>
            ...
            <tbody>
              {(jvView.postings || []).map((p, i) => (<tr key={i} style={{ borderBottom: '1px solid #dfe2e7' }}><td style={{ padding: '5px 8px', fontWeight: 600, color: DARK }}>{p.ledger}</td><td style={{ padding: '5px 8px', color: DIM }}>{p.group}</td><td style={{ padding: '5px 8px', textAlign: 'right', fontVariantNumeric: 'tabular-nums', color: p.debit ? BLUE : '#9aa3bd' }}>{p.debit ? money(cur, p.debit) : cur + '0'}</td><td style={{ padding: '5px 8px', textAlign: 'right', fontVariantNumeric: 'tabular-nums', color: p.credit ? RED : '#9aa3bd' }}>{p.credit ? money(cur, p.credit) : cur + '0'}</td></tr>))}
            </tbody>
