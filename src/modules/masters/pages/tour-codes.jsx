          ...
          <div key={c.id} className={`overflow-hidden rounded-brand border border-surface-border bg-surface shadow-sm ${c.active ? '' : 'opacity-60'}`}>
            ...
            <div className="px-3.5 py-2.5">
              ...
              <div className="mt-2.5 flex gap-1.5">
                ...
                <Button size="xs" variant="secondary" className={c.active ? 'text-maroon' : 'text-[#16a34a]'} onClick={() => setCodes((cs) => cs.map((x) => x.id === c.id ? { ...x, active: !x.active } : x))}>{c.active ? 'Archive' : 'Restore'}</Button>
              </div>
            </div>
          </div>
        ))}
