...
function FinancialTables({ mods = [], assets = [], liabs = [], rec, pay, recRows = [], payRows = [], bankRows = [], liquidFallback = 0, dateTo, fmt, nav }) {
  ...
  return (
    <>
      ...
      <div className="mb-3.5 grid grid-cols-1 gap-3.5 tablet:grid-cols-2">
        <WidgetCard title="Receivables — Ageing & Settlement" subtitle="Bills · settled · unsettled · ageing by sub-group" color="#dc2626" onDrill={() => nav('/reports/rec?tab=settlement')}>
          <ArApSettlementView side="receivable" totals={rec || {}} rows={recRows} formatMoney={fmt} collapsed />
        </WidgetCard>
        <WidgetCard title="Payables — Ageing & Settlement" subtitle="Bills · settled · unsettled · ageing by sub-group" color="#16a34a" onDrill={() => nav('/reports/pay?tab=settlement')}>
