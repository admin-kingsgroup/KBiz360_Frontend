...
export const DEV_REGISTRY = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'Recurring vouchers / Multi-currency voucher / Print-preview & Comments demos', status: 'live', modules: ['vouchers'], routes: ['/accounting/recurring', '/finance/multi-currency', '/finance/print-preview', '/finance/comments-demo'], api: ['/api/recurring-vouchers', '/api/vouchers'], note: 'BUILT FOR REAL 2026-07-10. Recurring: /api/recurring-vouchers templates + daily cron (RECURRING_CRON, default 02:15) — every occurrence is a PENDING voucher on the JV/RV/PMT series (approver signs off; scheduler can never write books alone). Multi-currency: converts at the live forex rate and posts ONE balanced 4-line JV (Dr Customer/Cr Sales · Dr Cost/Cr Supplier), pending. Comments: persisted append-only thread on the voucher (GET/POST /api/vouchers/:id/comments). Print-preview: formal print layout of any REAL voucher by number (PENDING banner; posting stays in Approvals). Restart backend to mount.' },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'FX revaluation / Intercompany billing', status: 'live', modules: ['forex-rates', 'interbranch-reconciliation'], routes: ['/accounting/fx-revaluation', '/accounting/intercompany'], api: ['/api/accounting/fx-revaluation'], note: 'BUILT 2026-07-10. FX: per-CURRENCY revaluation of consolidated exposure vs the last revaluation\'s rates (branch books are single-currency, so ledger-level FCY revaluation cannot exist); first run records the baseline, later runs post ONE pending JV in HO books (FX Revaluation Adjustment ↔ FCTR, equity not P&L); idempotent per period (FxRevaluation snapshot). Intercompany: retired the NotWired screen — /accounting/intercompany now opens the live INB register. Restart backend to mount.' },
    ],
  },
  {
    ...
    items: [
      ...
      { name: 'Statutory Filing Register + Vacation Delegations screens', status: 'stub', modules: ['tax-calendar', 'user-access'], remark: 'Both render static demo constants (STATUTORY_FILINGS / ACTIVE_DELEGATIONS in ho-control/legacy.jsx). Filing register should read/write taxcalendarevents (already a Task List item); delegations need a small collection + approval hook. Until wired, they are display-only.', routes: ['/settings/filing-register', '/settings/delegations'], note: 'VERIFIED 2026-07-10: PHASE2_Page stubs re-homed from HO Control — no API calls anywhere; buttons are visual only.' },
