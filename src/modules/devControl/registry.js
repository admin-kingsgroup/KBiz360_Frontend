...
export const DEV_REGISTRY = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'Recurring vouchers / Print-preview & Comments demos', status: 'live', modules: ['vouchers'], routes: ['/accounting/recurring', '/finance/print-preview', '/finance/comments-demo'], api: ['/api/recurring-vouchers', '/api/vouchers'], note: 'BUILT FOR REAL 2026-07-10. Recurring: /api/recurring-vouchers templates + daily cron (RECURRING_CRON, default 02:15) — every occurrence is a PENDING voucher on the JV/RV/PMT series (approver signs off; scheduler can never write books alone). Comments: persisted append-only thread on the voucher (GET/POST /api/vouchers/:id/comments). Print-preview: formal print layout of any REAL voucher by number (PENDING banner; posting stays in Approvals). MULTI-CURRENCY VOUCHER REMOVED 2026-07-16: it converted at a forex rate and posted a JV — forex reaching the books, which the group does not do (independent branches, each booking in its OWN currency, never converted or revalued). Forex rates are local-print only.' },
    ],
  },
  {
