/* ════════════════════════════════════════════════════════════════════
   ...
   Consolidated (branch='ALL'): the hooks return a `byBranch` slice on each
   payload (gp-bills as a `.byBranch` array PROPERTY, module-P&L + ageing as a
   `data.byBranch` array), each slice carrying the same shape as the merged
   top-level. We render EACH branch as its own MIS block in its OWN currency —
   never a merged cross-branch / cross-currency money total.
   ──────────────────────────────────────────────────────────────────── */
...
export function MisReport({ branch }) {
  ...
  // Consolidated = all-branches scope: render each branch's MIS block in its OWN
  // currency, never a merged cross-currency total. Driven by the BE `byBranch`
  // slices that carry the same shape as the merged top-level payloads.
  ...
  // One branch's full MIS body — all KPI / trend / leaderboard / ageing math + JSX,
  // computed from ONLY that scope's data, in `sCur` (its own currency). `sBills` is
  // the all-time GP list for the scope, `sPL` the module-P&L payload, `sAg` the ageing
  // payload. In single-branch mode this is the merged top-level data; in consolidated
  // mode each branch's own `byBranch` slice is fed in (never summed across branches).
  const renderBody = (sBills, sPL, sAg, sCur) => {
    const f = (n) => compactAmt(n, { currency: sCur }); // canonical compact (branch currency)
    const pct = (val, total) => (total > 0 ? Math.round(val / total * 100) : 0);

    const bills = sBills.filter((b) => String(b.date).startsWith(period));
    const prev = sBills.filter((b) => String(b.date).startsWith(prevMonthKey(period)));

    const rev = bills.reduce((s, b) => s + (+b.sell || 0), 0);
    const cost = bills.reduce((s, b) => s + (+b.cost || 0), 0);
    const gp = rev - cost;
    const gpPct = rev > 0 ? +(gp / rev * 100).toFixed(1) : 0;
    const exp = sPL ? ((sPL.indirect && sPL.indirect.expense) || 0) : 0;
    const netPft = gp - exp;
    ...
    const overdueClients = ((sAg && sAg.receivables && sAg.receivables.rows) || [])
    ...
    const trendData = trendMonths.map((m) => { const mb = sBills.filter((b) => String(b.date).startsWith(m)); const mr = mb.reduce((s, b) => s + (+b.sell || 0), 0); const mg = mb.reduce((s, b) => s + ((+b.sell || 0) - (+b.cost || 0)), 0); return { m, rev: mr, gp: mg, gpPct: mr > 0 ? +(mg / mr * 100).toFixed(1) : 0 }; });
  ...
  // Consolidated branch-wise: join the three byBranch slices (gp-bills, module-P&L,
  // ageing) by branch code, and render each branch as its own block in its own
  // currency. NO cross-branch / cross-currency money total. The merged top-level
  // payloads are intentionally ignored on this path.
  const gpByBranch = Array.isArray(GP_DATA.byBranch) ? GP_DATA.byBranch : null;
  const consolidated = isAll && gpByBranch;
  const plBy = (code) => (Array.isArray(plQ.data?.byBranch) ? plQ.data.byBranch.find((x) => x.branch === code) : null);
  const agBy = (code) => (Array.isArray(agQ.data?.byBranch) ? agQ.data.byBranch.find((x) => x.branch === code) : null);
  ...
  return (
    ...
    >
      {consolidated
        ? (gpByBranch.length === 0
          ? <div className="rounded-brand border border-surface-border bg-surface px-4 py-10 text-center text-[12.5px] text-ink-muted">No bookings in any branch.</div>
          : gpByBranch.map((b) => (
            <div key={b.branch} className="mb-6">
              <div className="mb-2.5 flex items-baseline gap-2 border-b-2 border-[#2563eb] pb-1">
              ...
              {renderBody(b.rows || [], plBy(b.branch), agBy(b.branch), curOf(b.branch))}
            </div>
          )))
        : renderBody(GP_DATA, plQ.data, agQ.data, cur)}
    </PageLayout>
  );
}
