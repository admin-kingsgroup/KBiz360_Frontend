/* ════════════════════════════════════════════════════════════════════
   ...
   OWN currency — never a merged cross-branch / cross-currency money total. The
   whole report body is extracted into a `section(rawBills, cur, branchCode)`
   helper; single-branch calls it once (byte-for-byte unchanged), consolidated
   loops `gpQuery.data.byBranch` and renders one section per branch slice.
   ──────────────────────────────────────────────────────────────────── */
...
import { MODULE_ICONS, CONSOLIDATED_LABEL, BRANCHES } from '../../../core/data';
...
export function ReportGP({ branch }) {
  ...
  // Consolidated = all-branches scope: render each branch as its own GP section in
  // its OWN currency, never a merged cross-currency total. Driven by the BE `byBranch`
  // slice ([{ branch, rows, totals }]) attached as a PROPERTY on the bills array.
  const isAll = !branch || branch === 'ALL' || branch?.code === 'ALL';
  // byBranch slices carry a bare branch CODE string; `bc` resolves on `.code`, so wrap
  // it into { code } to get each branch's OWN currency (a bare string would fall back to
  // BOM). Label = the branch's city from the reference list, else the code itself.
  ...
  // Per-scope bill normaliser: applies the client-side date/module/search filters and
  // derives gp / gpPct. In single-branch mode it filters by branch?.code; in a byBranch
  // slice the rows are ALREADY that branch's, so brCode is null (no extra branch filter).
  const normalise = (rawRows, brCode) => {
  ...
  // ── One scope's full report body, in `secCur`. Money totals NEVER cross this scope;
  //    each branch section sums only its own bills in its own currency. Tax columns are
  //    not present here (GP report is revenue/cost/GP only). The pivot tab toggle (`tab`)
  //    is global and applied uniformly to every section.
  const section = (rawRows, secCur, brCode) => {
    const bills = normalise(rawRows, brCode);
    const money = (n) => secCur + f(n);
    ...
    const monthly = monthlyOf(bills);
    const quarterly = quarterlyOf(bills);
    ...
    const periodRows = rows.map((m, i) => { const prev = rows[i - 1]; const delta = prev && prev.gpPct > 0 ? +(m.gpPct - prev.gpPct).toFixed(1) : null; return { ...m, delta }; });
    const periodColumns = [
  ...
  // Single-branch subtitle stats (the consolidated header is per-branch, so it omits a
  // merged money total). Computed only when NOT consolidated to keep behaviour identical.
  const singleBills = useMemo(() => (isAll ? [] : normalise(GP_DATA, branch?.code)), [GP_DATA, branch, dateFrom, dateTo, modFilter, search, isAll]);
  const sTotSell = singleBills.reduce((s, b) => s + b.sell, 0);
  const sTotGP = singleBills.reduce((s, b) => s + b.gp, 0);
  const sTotGPPct = sTotSell > 0 ? +(sTotGP / sTotSell * 100).toFixed(1) : 0;
  const singleMoney = (n) => cur + f(n);

  const byBranch = Array.isArray(GP_DATA.byBranch) ? GP_DATA.byBranch : null;

  // Export uses the in-scope merged array (date filters applied by the query). On the
  // consolidated path this is a flat all-branch dump for Excel only — the on-screen
  // money never crosses currencies; Excel rows still carry each row's own branch.
  const exportRows = isAll ? normalise(GP_DATA, null) : singleBills;
  ...
  return (
    ...
    >
      ...
      {/* CONSOLIDATED: each branch its own section, in its own currency — never merged.
          A consolidated fetch with no rows omits byBranch entirely → empty-state. */}
      {!gpQuery.isLoading && !gpQuery.error && isAll && !byBranch && (
        <PageSection><div className="py-12 text-center"><p className="text-3xl">📊</p><p className="mt-2 text-sm font-bold text-navy">No GP data in any branch for the selected period</p></div></PageSection>
      )}
      {!gpQuery.isLoading && !gpQuery.error && isAll && byBranch && (
        byBranch.length === 0
          ? <PageSection><div className="py-12 text-center"><p className="text-3xl">📊</p><p className="mt-2 text-sm font-bold text-navy">No GP data in any branch for the selected period</p></div></PageSection>
          : byBranch.map((b) => (
            <div key={b.branch} className="mb-7">
              ...
              {section(b.rows, curOf(b.branch), null)}
            </div>
          ))
      )}

      {/* SINGLE BRANCH: unchanged single-scope body. */}
      {!gpQuery.isLoading && !gpQuery.error && !isAll && (
        singleBills.length === 0
          ? <PageSection><div className="py-12 text-center"><p className="text-3xl">📊</p><p className="mt-2 text-sm font-bold text-navy">No GP data available for the selected period</p><p className="mt-1.5 text-[11px] text-ink-muted">Try widening the date range, or changing the branch / module filter.</p></div></PageSection>
          : section(GP_DATA, cur, branch?.code)
      )}
    </PageLayout>
  );
}
