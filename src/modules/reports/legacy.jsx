        ...
        <div style={{background:"#fff",border:"1px solid #e6e8ec",borderRadius:8,overflow:"hidden",maxHeight:520,overflowY:"auto"}}>
          ...
          {Object.entries(BUILDER_FIELD_CATALOG).map(([cat,fields])=>(
            <div key={cat}>
              ...
              {fields.map(f=>(
                <div key={f} onClick={()=>addField(f)} style={{padding:"7px 12px",fontSize:11.5,cursor:"pointer",color:selected.includes(f)?"#5b616e":"#1a1c22",background:selected.includes(f)?"#f0f2f7":"#fff",borderBottom:"1px solid #f7f8fb",display:"flex",justifyContent:"space-between",alignItems:"center"}}
            ...
            <div style={{padding:12,display:"flex",flexWrap:"wrap",gap:8,minHeight:52}}>
              {selected.map((f,i)=>(
                <div key={f} style={{display:"flex",alignItems:"center",gap:4,padding:"5px 8px",background:"#1a1c22",borderRadius:5,fontSize:11.5,color:"#fff",fontWeight:600}}>
                  <span style={{cursor:"pointer",color:"#5b616e",fontSize:12}} onClick={()=>moveField(f,-1)}>◀</span>
                  <span>{f}</span>
                  <span style={{cursor:"pointer",color:"#5b616e",fontSize:12}} onClick={()=>moveField(f,1)}>▶</span>
                  <span onClick={()=>removeField(f)} style={{cursor:"pointer",color:"#d97706",marginLeft:4,fontSize:13,fontWeight:700}}>×</span>
                </div>
              ))}
